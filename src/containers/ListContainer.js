...
const ListContainer = (props) => {
  ...
  let sortButtons = (
    <div>
      <button
        className="sort-button" onClick={() => props.sortAlphabetically("toDoList")}>Sort Alphabetically
      </button>
      <button
        className="sort-button" onClick={() => props.sortChronologically("toDoList")}>Sort Chronologically
      </button>
    </div>
  );
  ...
  let toDoContainer = (
      <div>
          ...
          {props.list.length > 0 ? sortButtons : <div></div>}
      </div>
    );
